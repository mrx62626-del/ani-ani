import { useEffect } from 'react';import { useParams } from 'react-router-dom';import { Home, ChevronRight } from 'lucide-react';import { usePlayer } from '../features/player/hooks/usePlayer';

// Feature Componentsimport EpisodeList from '../features/player/components/EpisodeList';import VideoPlayer from '../features/player/components/VideoPlayer';import PlayerControls from '../features/player/components/PlayerControls';import { useTitleLanguage } from '../context/TitleLanguageContext';import { getDisplayTitle } from '../utils/titleLanguage';import { getAnimeDetailsRouteId } from '../utils/animeNavigation';

export default function WatchPage() {const { id, title } = useParams<{ title: string; id: string }>();const { language } = useTitleLanguage();const extractDirectScraperSession = (value: unknown): string => {const raw = String(value || '').trim();const normalized = raw.replace(/^s:/i, '').replace(/^https?://[^/]+/i, '').replace(/^/+/, '').replace(/^watch//i, '').trim();if (!normalized) return '';return /^\d+$/.test(normalized) ? '' : normalized;};

useEffect(() => {document.documentElement.classList.add('watch-safe-mode');document.body.classList.add('watch-safe-mode');return () => {document.documentElement.classList.remove('watch-safe-mode');document.body.classList.remove('watch-safe-mode');};}, []);

// ✅ ADD HERE (exact position)useEffect(() => {const triggerPopunder = () => {if ((window as any).__popunderLoaded) return;(window as any).__popunderLoaded = true;
...
return () => {document.removeEventListener('click', triggerPopunder);};}, []);
